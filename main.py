...
def preprocess_data(df):
    ...
    # 6. Label Encoding para Disease (para uso futuro, se necessário)
...
def create_severity_labels_improved(data, aux_data=None):
    ...
    - Possíveis dados de severidade auxiliando
    ...
    for _, row in data.iterrows():
        ...
        if (
            row.get('Difficulty Breathing', 0) == 1
            and row.get('Fever', 0) == 1
            and row.get('Age', 0) >= 60
        ):
            severity.append(2)
            continue
        ...
        if (
            row.get('Difficulty Breathing', 0) == 1
            and row.get('Fever', 0) == 1
            and row.get('Fatigue', 0) == 1
        ):
            severity.append(2)
            continue

        # REGRA 5: Idosos com sintomas respiratórios (Emergência)
        if row.get('Age', 0) >= 70 and row.get('Difficulty Breathing', 0) == 1:
            severity.append(2)
            continue
        ...
        risk_factors = 0
        bp = row.get('Blood Pressure', np.nan)
        chol = row.get('Cholesterol Level', np.nan)
        age = row.get('Age', np.nan)

        if not pd.isna(bp) and bp == 2:
            risk_factors += 1
        if not pd.isna(chol) and chol == 2:
            risk_factors += 1
        if not pd.isna(age) and age >= 50:
            risk_factors += 1
...
def normalize_features(X_train, X_test):
    ...
    print(f"✓ Range (treino): [{X_train_normalized.min():.2f}, {X_train_normalized.max():.2f}]")
...
class ANFISLayer(layers.Layer):
    """
    Camada de fuzzificação com funções de pertinência Gaussianas.
    Aproxima a primeira camada do ANFIS descrito no artigo:
    cada variável de entrada recebe 'num_mfs' funções gaussianas.
    """
    ...
    def call(self, inputs):
        ...
        for i in range(self.num_inputs):
            x = tf.expand_dims(inputs[:, i], axis=-1)          # (batch, 1)
            mu = self.mu[i:i + 1, :]                           # (1, num_mfs)
            sigma = tf.abs(self.sigma[i:i + 1, :]) + 1e-6      # evitar sigma=0

            membership = tf.exp(-tf.square(x - mu) / (2.0 * tf.square(sigma)))
            memberships.append(membership)

        # Concatena todas as pertinências (todas as variáveis) em um único vetor
        return tf.concat(memberships, axis=-1)


def build_anfis_model(input_dim, num_mfs=3, num_rules=27, num_classes=3):
    """
    Constrói o modelo ANFIS aproximado conforme descrito no artigo:

    1) Fuzzificação (Gaussianas)
    2) Regras (camada densa + ReLU)
    3) Normalização (BatchNormalization)
    4) Consequente TSK (Dense + L2 + Dropout)
    5) Saída (Softmax)
    """
    print("\n" + "=" * 50)
    ...
    # Camada 2: Regras (combinação das pertinências)
    ...
    # Camada 4: Consequente (TSK) - com regularização L2 e dropout, como no PDF
    ...
    consequent = layers.Dropout(0.5)(consequent)
    consequent = layers.Dense(
        32,
        activation='relu',
        kernel_regularizer=keras.regularizers.l2(0.01),
...
def plot_results(history, cm):
    """Visualiza os resultados (loss, acurácia, matriz de confusão)"""
...
def main():
    ...
    print(" Versão 2.0 - Multi-Dataset (alinhado ao artigo)")
    print("=" * 60)
    ...
    # 7. Dividir em treino, validação e teste (64% / 16% / 20%)
    ...
    class_weights = {cls: w for cls, w in zip(classes_unique, class_weights_array)}
    ...
    # 15. Salvar scaler e encoder (para uso futuro na aplicação)
    import joblib
